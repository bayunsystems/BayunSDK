...
int testJoinPublicGroup(ShTestArgs test, std::string sessionId, std::string groupId, std::string creatorCompanyName, std::string creatorCompanyEmpId) {
  LOG(LOG_INFO, stderr, "---------------------------------------------------------\n");
  LOG(LOG_INFO, stderr, "Running Test Join Public Group\n");
  LOG(LOG_INFO, stderr, "---------------------------------------------------------\n");
  try{
    Bayun::ShJoinPublicGroupResponse joinPublicGroupResp = test->getBayunCore()->joinPublicGroup(sessionId, groupId, creatorCompanyName, creatorCompanyEmpId);
...
int main(int argc, char *argv[]) {
  try {
    ...
    Bayun::ShBayunCore bayunCore =  Bayun::BayunCore::getInstance(baseURL, appId, appSecret, appSalt, "");
    ...
    std::vector<Bayun::ShSecurityQuestionAnswer> answers;
    answers.push_back(std::make_shared<Bayun::SecurityQuestionAnswer>("1", "q", "a"));
    answers.push_back(std::make_shared<Bayun::SecurityQuestionAnswer>("2", "q", "a"));
    answers.push_back(std::make_shared<Bayun::SecurityQuestionAnswer>("3", "q", "a"));
    answers.push_back(std::make_shared<Bayun::SecurityQuestionAnswer>("4", "q", "a"));
    answers.push_back(std::make_shared<Bayun::SecurityQuestionAnswer>("5", "q", "a"));
    
    testLoginWithPassword(test,"27j1.com","test1","1234","1234",answers);
