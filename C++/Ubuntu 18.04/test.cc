...
/**
 ...
 @return Status.
 */
int testJoinPublicGroup(ShTestArgs test, std::string sessionId, std::string groupId) {
  LOG(LOG_INFO, stderr, "---------------------------------------------------------\n");
  LOG(LOG_INFO, stderr, "Running Test Join Public Group\n");
  LOG(LOG_INFO, stderr, "---------------------------------------------------------\n");
  try{
    Bayun::ShJoinPublicGroupResponse joinPublicGroupResp = test->getBayunCore()->joinPublicGroup(sessionId, groupId);
