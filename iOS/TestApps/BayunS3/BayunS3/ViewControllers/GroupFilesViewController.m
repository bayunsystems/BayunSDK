...
- (void)viewDidLoad {
  ...
  self.bucketName = [NSString stringWithFormat:@"bayun-group-%@",self.group.groupId];
...
- (void)createS3Bucket {
  ...
  [awsManagerInstance createS3BucketWithName:[self.bucketName lowercaseString] success:^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (![[userInfo valueForKey:@"Message"] isEqual:[NSNull null]]) {
        [SVProgressHUD showErrorWithStatus:[userInfo valueForKey:@"Message"]];
      } else {
...
- (void)getS3BucketObjects {
  ...
  if (appDelegate.isNetworkReachable) {
    ...
    [awsManagerInstance getBucketFiles:self.bucketName success:^(AWSS3ListObjectsOutput *bucketObjectsList) {
      ...
      if([[error.userInfo valueForKey:@"Code"] isEqualToString:@"NoSuchBucket"]) {
        [self createS3Bucket];
      } else {
  ...
  [alertView showWithCompletion:^(DLAVAlertView *alertView, NSInteger buttonIndex) {
    if (buttonIndex == 1) {
      ...
      if ([companyName stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] &&
        ...
        GroupMember *groupMember = [[GroupMember alloc] initWithCompanyName:companyName
                                                          companyEmployeeId:companyEmployeeId];
        if ([title isEqualToString:@"Add Member"]) {
          
          [[BayunCore sharedInstance] addInGroup:groupId
                                     groupMember:groupMember
                                         success:^(NSArray<AddMemberErrObject *> *errorObjects, NSString *addedMembersCount) {
            if (![addedMembersCount isEqualToString:@"0"]) {
              [SVProgressHUD showSuccessWithStatus:kMemberAddedSuccessfully];
            } else {
              if (errorObjects.count > 0) {
                [SVProgressHUD showErrorWithStatus:errorObjects[0].errorMessage];
              } else {
                [SVProgressHUD showSuccessWithStatus:kErrorMsgSomethingWentWrong];
              }
            }
...
- (void)showAlertViewToLeaveGroup {
  ...
  UIAlertAction* yesButton = [UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
    ...
    [[BayunCore sharedInstance] leaveGroup:self.group.groupId success:^{
