...
@interface ListFilesViewController ()<AWSManagerDelegate,UIDocumentPickerDelegate,UIDocumentMenuDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,QLPreviewControllerDataSource,QLPreviewControllerDelegate,UIDocumentInteractionControllerDelegate>
...
/**
 Lists all the files in S3 bucket
 */
...
- (void)addNavigationBarButton {
    self.createButton = [[UIBarButtonItem alloc] initWithTitle:@"Create"
...
- (void)createS3BucketWithName {
    ...
    NSString *bucketName = [NSString stringWithFormat:@"bayun-%@",[[NSUserDefaults standardUserDefaults] valueForKey:kCompanyName]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.detailTextLabel.text = [NSString stringWithFormat:@"Size:%@  Last Modified: %@",
                                 [Utilities transformedFileSizeValue:[s3Object valueForKey:@"size"]],
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[AWSManager sharedInstance] downloadFileAtPath:filePath];
    ...
    self.previewController.currentPreviewItemIndex = self.tableView.indexPathForSelectedRow.row;
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url {
    ...
    if ([[fileExtension uppercaseString] isEqualToString:@"JPG"]) {
        UIImage *image = [UIImage imageWithData: [NSData dataWithContentsOfURL:url]];
        fileData = UIImageJPEGRepresentation(image, 0.8);
    } else if([[fileExtension uppercaseString] isEqualToString:@"PNG"]) {
    ...
    [[AWSManager sharedInstance] uploadFileFromPath:filePath];
}
