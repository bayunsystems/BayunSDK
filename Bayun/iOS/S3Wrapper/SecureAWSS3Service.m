        ...
        if (task.result) {
            ...
            [[BayunCore sharedInstance] decryptFileAtPath:encryptedFilePath success:nil failure:^(NSUInteger errorCode){
                if (errorCode == BayunErrorAccessDenied) {
                    ...
                    taskNew = [AWSTask taskWithError:[NSError errorWithDomain:SecureAWSS3ServiceErrorDomain code:SecureAWSS3ServiceErrorUserInactive userInfo:userInfo]];
                } else {
...
- (AWSTask *)putObject:(AWSS3PutObjectRequest *)request {
    ...
    [[BayunCore sharedInstance] encryptFileAtPath:filePathString success:^{
        ...
        if (errorCode == BayunErrorAccessDenied) {
            ...
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: NSLocalizedString(@"User Inactive", nil)};
            task = [AWSTask taskWithError:[NSError errorWithDomain:SecureAWSS3ServiceErrorDomain code:SecureAWSS3ServiceErrorUserInactive userInfo:userInfo]];
        } else {
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: NSLocalizedString(@"Something Went Wrong", nil)};
            task = [AWSTask taskWithError:[NSError errorWithDomain:SecureAWSS3ServiceErrorDomain code:SecureAWSS3ServiceErrorSomethingWentWrong userInfo:userInfo]];
        }
    }];
        ...
        for (NSUInteger i = c; i < partCount + 1; i++) {
            uploadPartsTask = [uploadPartsTask continueWithSuccessBlock:^id(AWSTask *task) {
                ...
                if (uploadRequest.state == AWSS3TransferManagerRequestStateCanceling) {
                    ...
                    return [AWSTask taskWithError:[NSError errorWithDomain:SecureAWSS3ServiceErrorDomain code:SecureAWSS3ServiceErrorCancelled userInfo:userInfo]];
                }
                ...
                if (uploadRequest.state == AWSS3TransferManagerRequestStatePaused) {
                    ...
                    return [AWSTask taskWithError:[NSError errorWithDomain:SecureAWSS3ServiceErrorDomain code:SecureAWSS3ServiceErrorPaused userInfo:userInfo]];
                }
