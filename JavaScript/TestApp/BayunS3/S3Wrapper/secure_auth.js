class SecureAuthentication {
    ...
    register = async (sessionId, companyEmployeeId, email, password, registerBayunWithPwd) => {
    ...
    login = async (sessionId, companyEmployeeId, email, password, loginWithPwd) => {

        var session_Id;
        if (loginWithPwd == true) {
            session_Id = await signIn(sessionId, true, companyEmployeeId, email, password);
        }
        else {
            session_Id = await signIn(sessionId, false, companyEmployeeId, email, password);
        }
        return session_Id;
    }
...
async function signUp(sessionId, registerBayunWithPwd, personalname, email, password) {
    ...
    userPool.signUp(email, password, attributeList, null, function (err, result) {
        ...
        registerSuccessHandler(registerBayunWithPwd, cognitoUser.getUsername(), bayunData);
...
async function signIn(session_Id, LoginWithPwd, personalname, email, password) {
    ...
    let sessionId;
    await authenticateUser(cognitoUser, authenticationDetails, LoginWithPwd, bayunData).then((session)=>{
        sessionId = session;
    }).catch((error)=>{
        console.log(error);
    });
...
async function authenticateUser(cognitoUser, authenticationDetails, LoginWithPwd, bayunData) {
    return new Promise((resolve, reject) => {
        cognitoUser.authenticateUser(authenticationDetails, {
            onSuccess: async function (result) {
                ...
                let session = await signInSuccessHandler(accessToken, LoginWithPwd, bayunData);
                resolve(session);
            },
...
async function registerSuccessHandler(registerBayunWithPwd, email, bayunData) {
    if (email != null) {
        if (registerBayunWithPwd) {
            await registerBayun(bayunData.sessionId, bayunCompanyName, bayunData.companyEmployeeId, bayunData.email, bayunData.password, true);
        } else {
            await registerBayun(bayunData.sessionId, bayunCompanyName, bayunData.companyEmployeeId, bayunData.email, bayunData.password, false);
        }
    }
}
...
async function signInSuccessHandler(accessToken, LoginWithPwd, bayunData) {
    ...
    if (accessToken != null) {
        if (LoginWithPwd) {
            sessionid = await loginBayun(bayunData.session_Id, bayunCompanyName, bayunData.companyEmployeeId, bayunData.password, true);
        } else {
            sessionid = await loginBayun(bayunData.session_Id, bayunCompanyName, bayunData.companyEmployeeId, bayunData.password, false);
        }
    }
    return sessionid;
}

async function registerBayun(sessionId, bayunCompanyName, companyEmployeeId, email, password, withPassword) {
    if (withPassword) {
...
async function loginBayun(sessionId, bayunCompanyName, companyEmployeeId, password, withPassword) {
    if (withPassword) {
