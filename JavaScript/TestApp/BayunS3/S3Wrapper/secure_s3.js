...
class SecureS3 {
    constructor(
        ...
        if (bayunSessionId && apiVersion && accessKeyId && secretAccessKey && signatureVersion && region && bucket) {
    ...
    downloadImage = async (utilCall, name, DownloadCallBack, unlockTextCallBack, bayunSessionId) => {
        let data;
        try {
        ...
        try {
            var unlocked_Text = await unlockTextCallBack(bayunSessionId, Base64String);
    ...
    writeFileTextToFile = async (fileText, fileType, name) => {
        ...
        var extension = fileType.substring(fileType.indexOf('/') + 1, fileType.length);
    ...
    unlockTextForImage = async (sessionId, text) => {
        var unlockedText = await bayunCoreObjectS3.unlockFileText(sessionId, text).then(res => res);
        return unlockedText;
    }
    ...
    proccedUpload = async (textToLock, type, name) => {
        ...
        await this.uploadToS3Bucket(lockedBase64ToArrayBuffer, (progress) => {
        ...
        await this.downloadImage(this.util, name, this.writeFileTextToFile, this.unlockTextForImage, this.bayunSessionId);
    }
